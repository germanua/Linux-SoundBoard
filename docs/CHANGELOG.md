...
- **Show the playing sound in the desktop's media controls:** `System Tray` → `Show In Media Controls` publishes the current sound to the panel's media widget with working transport buttons, and on GNOME this works without any extension. Off by default: while a sound plays the app becomes the active media player, which takes the media keys with it, so the player is published only for as long as a sound is actually playing and released the moment it stops.
