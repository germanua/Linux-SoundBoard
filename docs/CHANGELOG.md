...
- **The playing dot kept the previous theme's colour:** Its pulse animated `color` between two hardcoded values. Switching the theme replaces the stylesheet but does not change the animation's name, so GTK kept the running animation and its original keyframes, and the dot went on drawing itself in the colour of the theme that had just been left. The pulse now animates opacity alone and takes its colour from the class, which resolves again on every switch. This also makes the card-mode dot colour take effect, which the animation had been overwriting on every frame.
...
- **Downloads are verified:** every download — tarball, `.deb`, `.rpm`, and AppImage — is checked against the release's `SHA256SUMS.txt`. A missing list, missing entry, invalid hash, unavailable SHA-256 tool, or checksum mismatch stops the install. Every packaging script refreshes `dist/SHA256SUMS.txt` through `packaging/generate-checksums.sh`.
