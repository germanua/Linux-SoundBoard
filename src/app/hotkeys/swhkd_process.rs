...
impl SwhkdProcesses {
    ...
    pub fn spawn_swhkd() -> Result<Child, HotkeyError> {
    ...
    pub fn spawn_managed() -> Result<Self, HotkeyError> {
    ...
    pub fn attach_existing() -> Result<Self, HotkeyError> {
        let swhkd_pid = Self::get_swhkd_pid().ok_or_else(|| {
            HotkeyError::Process("swhkd is running but PID could not be determined".to_string())
        })?;
