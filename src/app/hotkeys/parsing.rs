...
impl HotkeyCode {
    ...
    pub fn from_user_token(raw: &str) -> Result<Self, HotkeyError> {
        let canonical = canonicalize_key_token(raw);
        Self::from_token(&canonical)
            .ok_or_else(|| HotkeyError::Parse(format!("Unsupported key: {raw}")))
    }
...
impl HotkeySpec {
    ...
    pub(crate) fn swhkd_string(&self) -> Result<String, HotkeyError> {
        let mut parts: Vec<String> = self
            .modifiers
            .iter()
...
pub fn parse_hotkey_spec(hk: &str) -> Result<HotkeySpec, HotkeyError> {
    ...
    for raw_part in hk.split('+') {
        ...
        if part.is_empty() {
            return Err(HotkeyError::Parse(
                "Hotkey contains an empty token".to_string(),
            ));
        }
        ...
        if let Some(modifier) = HotkeyModifier::from_token(part) {
            ...
            if key.replace(next_key).is_some() {
                return Err(HotkeyError::Parse(
                    "Hotkey must include only one non-modifier key".to_string(),
                ));
            }
        }
    }
    ...
    let key = key
        .ok_or_else(|| HotkeyError::Parse("Hotkey must include a non-modifier key".to_string()))?;
    Ok(HotkeySpec::new(modifiers, key))
}

pub fn canonicalize_hotkey_string(hk: &str) -> Result<String, HotkeyError> {
