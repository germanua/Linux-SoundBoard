...
pub fn engine_info() -> Result<EngineInfo, EngineIpcError> {
    engine_info_at(&engine_socket_path())
}

pub fn engine_info_at(path: &Path) -> Result<EngineInfo, EngineIpcError> {
    match send_request_to(path, EngineRequest::Info)? {
        ...
        EngineResponse::Error { message } => Err(EngineIpcError::Protocol(message)),
        other => Err(EngineIpcError::Protocol(format!(
            "Unexpected engine info response: {other:?}"
        ))),
    }
}
...
pub fn bind_engine_socket_at(path: &Path) -> Result<BindEngineSocket, EngineIpcError> {
    if path.exists() {
        if engine_responds_at(path) {
    ...
    fs::create_dir_all(dir).map_err(|e| {
        EngineIpcError::Io(format!(
            "Failed to create engine socket dir {}: {e}",
            dir.display()
        ))
    })?;
    ...
    let listener = UnixListener::bind(path).map_err(|e| {
        EngineIpcError::Io(format!(
            "Failed to bind engine socket {}: {e}",
            path.display()
        ))
    })?;
    fs::set_permissions(path, fs::Permissions::from_mode(0o600)).map_err(|e| {
        EngineIpcError::Io(format!(
            "Failed to protect engine socket {}: {e}",
            path.display()
        ))
    })?;
...
pub fn send_request(request: EngineRequest) -> Result<EngineResponse, EngineIpcError> {
...
) -> Result<EngineResponse, EngineIpcError> {
    let mut stream = UnixStream::connect(path).map_err(|e| {
        EngineIpcError::Io(format!(
            "Failed to connect to engine socket {}: {e}",
            path.display()
        ))
    })?;
    stream.set_read_timeout(Some(IPC_TIMEOUT)).map_err(|e| {
        EngineIpcError::Io(format!("Failed to set engine socket read timeout: {e}"))
    })?;
    stream.set_write_timeout(Some(IPC_TIMEOUT)).map_err(|e| {
        EngineIpcError::Io(format!("Failed to set engine socket write timeout: {e}"))
    })?;
...
) -> Result<(), EngineIpcError> {
    write_message(stream, response)
}

pub fn parse_request(line: &str) -> Result<EngineRequest, EngineIpcError> {
    serde_json::from_str(line)
        .map_err(|e| EngineIpcError::Protocol(format!("Invalid engine request: {e}")))
}
...
fn write_message<T: Serialize>(stream: &mut UnixStream, message: &T) -> Result<(), EngineIpcError> {
    serde_json::to_writer(&mut *stream, message)
        .map_err(|e| EngineIpcError::Protocol(format!("Failed to encode engine message: {e}")))?;
    stream
        .write_all(b"\n")
        .map_err(|e| EngineIpcError::Io(format!("Failed to write engine message: {e}")))?;
    stream
        .flush()
        .map_err(|e| EngineIpcError::Io(format!("Failed to flush engine message: {e}")))
}

fn read_response(stream: UnixStream) -> Result<EngineResponse, EngineIpcError> {
    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    let read = reader
        .read_line(&mut line)
        .map_err(|e| EngineIpcError::Io(format!("Failed to read engine response: {e}")))?;
    if read == 0 {
        return Err(EngineIpcError::Io(
            "Engine closed the socket without a response".to_string(),
        ));
    }
    serde_json::from_str(line.trim_end())
        .map_err(|e| EngineIpcError::Protocol(format!("Invalid engine response: {e}")))
}
