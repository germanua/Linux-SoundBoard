...
/// Convert measured loudness into a linear gain factor. When the file's
/// true-peak (dBTP) is known, the gain is clamped so the output never exceeds
/// `TRUE_PEAK_CEILING_DBTP` — prevents inter-sample clipping on the mic
/// output after boost.
