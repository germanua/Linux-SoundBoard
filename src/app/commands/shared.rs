...
{
    Ok(f(&mut config.lock()))
}

pub fn with_saved_config<F>(config: &Arc<Mutex<Config>>, f: F) -> Result<(), CommandError>
where
    F: FnOnce(&mut Config),
{
...
pub fn with_saved_config_result<F, R>(config: &Arc<Mutex<Config>>, f: F) -> Result<R, CommandError>
where
    F: FnOnce(&mut Config) -> Result<R, CommandError>,
{
    with_config_mut(config, |cfg| {
        let result = f(cfg)?;
        cfg.save().map_err(CommandError::config_save)?;
...
#[cfg(test)]
pub(crate) fn parse_theme(s: &str) -> Result<crate::config::Theme, CommandError> {
...
pub fn parse_auto_gain_mode(s: &str) -> Result<crate::config::AutoGainMode, CommandError> {
    match s.to_lowercase().as_str() {
...
pub fn parse_auto_gain_apply_to(s: &str) -> Result<crate::config::AutoGainApplyTo, CommandError> {
    match s.to_lowercase().as_str() {
...
pub fn validate_play_mode(s: &str) -> Result<PlayMode, CommandError> {
    match s.to_lowercase().as_str() {
...
/// thread via `MainContext::invoke` — no polling timer, no per-call OS thread.
/// Call it from the main thread only: `on_complete` is `!Send` and waits in a
/// thread-local.
